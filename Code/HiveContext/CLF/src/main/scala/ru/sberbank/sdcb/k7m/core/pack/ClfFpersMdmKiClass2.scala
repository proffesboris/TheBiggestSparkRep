...
class ClfFpersMdmKiClass2 (val spark: SparkSession, val config: Config)extends EtlLogger with EtlJob {
...
//Если ИНН не найден в МДМ, то тащим его из источника, разделяя по конкретным источникам
	...
	private val union_part1 = tmp_clf_mdm_dedubl_enriched.join(broadcast(tmp_clf_raw_keys_enriched), tmp_clf_mdm_dedubl_enriched("inn_clear") === tmp_clf_raw_keys_enriched("keys_inn"), "left")
		.select(
			tmp_clf_raw_keys_enriched("id")
			,tmp_clf_raw_keys_enriched("position_eks")
			,tmp_clf_raw_keys_enriched("position_ul")
			,tmp_clf_raw_keys_enriched("doc_ser_eks")
			,tmp_clf_raw_keys_enriched("doc_num_eks")
			,tmp_clf_raw_keys_enriched("doc_date_eks")
			,tmp_clf_raw_keys_enriched("crm_id")
			,tmp_clf_raw_keys_enriched("inn_eks")
			,tmp_clf_raw_keys_enriched("inn_ul")
			,tmp_clf_raw_keys_enriched("inn_ex")
			,tmp_clf_raw_keys_enriched("inn_gs")
			...
			,tmp_clf_raw_keys_enriched("k1")
			,tmp_clf_raw_keys_enriched("k2")
			,tmp_clf_raw_keys_enriched("k3")
			,tmp_clf_raw_keys_enriched("k4")
	...
	private val unioned = union_part1.union(union_part2)
	...
	val dataframe = unioned.select(
		...
		when($"k1" === $"m1" && $"rn_id" === 1, lit(1))
		.when($"k2" === $"m2" && $"rn_tel" === 1, lit(2))
		.when($"k3" === $"m3" && $"rn_inn" === 1, lit(3))
		.when($"k4" === $"m4" && $"rn_inn_only" === 1, lit(4))
	  	.otherwise(lit(-1))
	)
