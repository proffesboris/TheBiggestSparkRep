...
class Dividends_IN(override val spark: SparkSession, val config: Config, date: LocalDate) extends EtlLogger with EtlJob {

  val stgSchema = config.stg
  val targetAuxSchema = config.aux
  val targetPaSchema = config.pa

  override val dashboardName = s"$targetAuxSchema.crit_dividends_in"
  override def processName: String = "SBL"
  val dashboardPath = s"${config.auxPath}crit_dividends_in"
  ...
  def run(): Unit = {
    ...
    val div_df_query = s"""
                     ...
                     select
                            ...
                            , '%s' as dt
                            , sum(v.c_sum_nt) as sum_dividends_in
                    ...
                    where (v.predicted_value in ('дивиденды'))
                         and (v.c_date_prov >= '%s')
                         and (v.c_date_prov < '%s')
                         and ((case when v.ktdt = 0 then v.inn_st else v.inn_sec end) in (select org_inn_crm_num inn from ${config.aux}.basis_client))
                         and ((case when v.ktdt = 0 then v.inn_sec else v.inn_st end) <> '%s')
                         and (filt = 0)
    ...
    val sql_query = s"""
                 ...
                 select
                             ...
                             ,case when re.revenue_L12M_true < 1 then 0
                                     else (sl.sum_dividends_in / re.revenue_L12M_true)
                                     end as quantity
