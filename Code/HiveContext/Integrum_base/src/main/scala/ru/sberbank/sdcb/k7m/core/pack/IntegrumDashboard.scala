...
class IntegrumDashboard(val spark: SparkSession, val dashboardName: String, val config: Config) extends EtlLogger with EtlJob {

  override def processName: String = "IntBase"


  def doCreateIntFinStmtRsbu(dateStr: String): Unit = {

    val intFinStmtRsbuTable = spark.sql(
      s"""select
      UL_INN as cust_inn_num
      ,cast(from_unixtime(unix_timestamp(PERIOD_START_DT, 'yyyy-MM-dd')) as timestamp) as fin_stmt_start_dt
      ,cast(add_months(cast(from_unixtime(unix_timestamp(PERIOD_START_DT, 'yyyy-MM-dd')) as timestamp), 12) as timestamp) as fin_stmt_rep_dt
    ...
    intFinStmtRsbuTable
      ...
      .format("parquet")
      .mode("overwrite")
      .option("path", s"${config.auxPath}int_fin_stmt_rsbu")
      .saveAsTable(s"${config.aux}.int_fin_stmt_rsbu")
