...
class FokDocsHeaderFinalLoanClass (val spark: SparkSession, val config: Config) extends EtlLogger with EtlJob {
  ...
  val Node2t_team_k7m_pa_d_fok_docs_header_final_loanIN = s"${Stg0Schema}.fok_docs_data"
   val Nodet_team_k7m_pa_d_fok_docs_header_final_loanOUT = s"${DevSchema}.fok_docs_header_final_loan"
  val dashboardPath = s"${config.auxPath}fok_docs_header_final_loan"
