package ru.sberbank.sdcb.k7m.core.pack.PRMTR3.OWNR_FL

import org.apache.spark.sql.expressions.{Window, WindowSpec}
import org.apache.spark.sql.functions._
import ru.sberbank.sdcb.k7m.core.pack.{Config, Main, Table}

class PRMTR3_ownr_fl(config: Config) extends Table(config: Config){

	val dashboardName: String = genDashBoardName(PRMTR3_ownr_fl_ShortName)
	val dashboardPath: String = genDashBoardPath(PRMTR3_ownr_fl_ShortName)
...
/*
	val dataframe = spark.sql(
		s"""
			 |SELECT
			 |        res.inn as inn1 --ИНН
			 |        , res.nm as nm -- ФИО (Добавил Семен)
