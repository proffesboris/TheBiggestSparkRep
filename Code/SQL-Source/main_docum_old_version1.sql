drop table if exists user_iyarukov.COUNTERP26DEF_jun2017;
create external table user_iyarukov.COUNTERP26DEF_jun2017 (
lnum string,
crm string,
INN string,
rat string,
...
stored as textfile
location '/user/iyarukov/jun_pred/inn/'
tblproperties("skip.header.line.count"="1");

create table  user_iyarukov.COUNTERP26DEF_jun2017_dist stored as orc as
select distinct inn from user_iyarukov.COUNTERP26DEF_jun2017;

select count(*),count(distinct inn) from  user_iyarukov.COUNTERP26DEF_jun2017_dist;

drop table if exists user_iyarukov.innimport010617_1;
create table user_iyarukov.innimport010617_1 stored as orc
as
select * 
from (select  id
  ,C_INN
  ,odsopc
  ,row_number() over (partition by id order by odsvalidfrom desc, odssequenceid desc, odsopc desc) as rn
  from core_internal_eks.Z_CLIENT  a 
  inner join user_iyarukov.COUNTERP26DEF_jun2017_dist  b on a.c_inn=b.inn
)t
 where t.rn=1 and t.odsopc<>'D'
;

create table user_iyarukov.innimport010617_2 stored as orc
as 
Select
a.* 
from (
                               select  
                                               ...
                                               ,z_account.c_name
                                               ,z_account.c_user_op
                                               ,z_account.c_client_v
                                               ,from_unixtime(cast(z_account.c_date_op / 1000 as int)) as c_date_op
                                               ,from_unixtime(cast(z_account.c_date_close / 1000 as int)) as c_date_close
                                               ,z_account.c_user_close
                                               ,z_account.c_products
                                               ,z_account.c_depart
                                               ,z_account.sn
                                               ,z_account.su
                                               ,from_unixtime(cast(z_account.odsvalidfrom / 1000 as int)) as odsvalidfrom
                                               ,z_account.odsopc 
                                               ,z_account.odssequenceid
                                               --для фильтрации истории добавим счетчик записей
                                               ,row_number() over (partition by id order by from_unixtime(cast(z_account.odsvalidfrom / 1000 as int)) desc) as r_num
                               from core_internal_eks.Z_ACCOUNT
                               where 1=1
                               ) a
where 1=1
--нужна только последняя версия 
and a.r_num = 1
and a.c_client_v in (Select distinct id from user_iyarukov.innimport010617_1)
;

create table user_iyarukov.innimport010617_spark stored as orc
as select 
v.id,
'0' as ktdt, 
v.sn,
...
case 
    ...
    when (substr(v.c_kl_dt_2_1,1,1) = '3' or substr(v.c_kl_dt_2_1,1,2) in ('40','41') or substr(v.c_kl_dt_2_1,1,5) = '47426')
          and  substr(v.c_kl_dt_2_1,1,5) <> '40906' 
          and coalesce(v.c_kl_dt_2_inn,'8') = coalesce(v.c_kl_kt_2_inn,'9')
          and mbik.FLDBIC is not null 
    then 'Пополнение.Сбер'

    when (substr(v.c_kl_dt_2_1,1,1) = '3' or substr(v.c_kl_dt_2_1,1,2) in ('40','41') or substr(v.c_kl_dt_2_1,1,5) = '47426')
          and  substr(v.c_kl_dt_2_1,1,5) <> '40906' 
          and coalesce(v.c_kl_dt_2_inn,'8') = coalesce(v.c_kl_kt_2_inn,'9')
           and mbik.FLDBIC is null   
...
from core_internal_eks.z_main_docum  v
inner join user_iyarukov.innimport010617_2  a
                on (a.id = v.c_acc_kt)
left join core_internal_eks.z_cl_bank_n plbic on plbic.id = v.c_kl_dt_2_3
left join (select distinct FLDBIC from  user_iyarukov.bik_num_id) mbik 
...
create table user_iyarukov.innimport010617_44 stored as orc as 
SELECT A.*
...
FROM user_iyarukov.innimport010617_spark  A 
...
LEFT JOIN core_internal_eks.Z_KOD_N_PAY E ON A.c_kod_nazn_pay=E.C_MEMO_NAZN
LEFT JOIN core_internal_eks.Z_ACC_OPER F ON A.c_acc_oper=F.ID
inner join user_iyarukov.innimport010617_1 g on a.c_client_v=g.id
...
create table user_iyarukov.innimport010617_5 stored as orc
as 
select * from user_iyarukov.innimport010617_44
where rn=1;
